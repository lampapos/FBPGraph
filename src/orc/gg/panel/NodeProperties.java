...
public class NodeProperties {
	...
	public JPanel generateJP(final ArrayList<Node> nodes, final int iter){
		...
		if(iter != -1){
			...
			JScrollPane hint = new JScrollPane(
					new JTextField(ComponentDescriptor.getComponentDescription(target.getClass()))
					);
			...
			jb.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(col.equals("default")){
						work.getGraph().setCellStyle(null, new Object[]{
									nodes.get(iter).cell
									}
								);
						nodes.get(iter).color = "";
					}else{
						work.getGraph().setCellStyles(
