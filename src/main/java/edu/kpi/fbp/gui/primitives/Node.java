...
public class Node {
   ...
   */
  public Node(final int id, final String className, final ComponentDescriptor componentDescriptor) {
    final String[] splitBuf = className.split("\\.");
   ...
   */
  public void addNewAttribute(final Parameter newParameter) {
   ...
   */
  public void setNewAttributes(final List<Parameter> newParameters) {
  ...
  public void draw(final mxGraph graph, final int x, final int y) {
    ...
    for (final InPort inPort : componentDescriptor.getInPorts()) {
    ...
    for (final OutPort outPort : componentDescriptor.getOutPorts()) {
    ...
    while (i < componentDescriptor.getInPorts().size()) {
      ...
      final int freeSpace = (maxWidth - inWidth) / componentDescriptor.getInPorts().size();
    ...
    while (i < cells.size()) {
      bufGeometry = graph.getCellGeometry(cells.get(i));
      bufGeometry.setHeight(height);
      final int freeSpace = (maxWidth - outWidth) / componentDescriptor.getOutPorts().size();
      bufGeometry.setWidth(bufGeometry.getWidth() + freeSpace);
      bufGeometry.setX(x + offcet);
      bufGeometry.setY(y + height);
  ...
  public String toString() {
    ...
    res +=  "   </ports>\n";
    res +=  "   <attribute>\n";
    for (final Parameter param : newAttribute) {
      res += "      " + param.toString() + "\n";
    }
    res +=  "   </attribute>\n";
    res +=  "   <links>\n";
