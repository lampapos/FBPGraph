...
public class NodeParams {
	...
	public NodeParams(mxGraphComponent comp, Connect con) {
	  this.work = comp;
	  this.conDes = con;
	}
	...
	private JPanel generateParams(final Node target) {
		...
		for (int i = 0; i < target.localParams.size(); i++) {
			...
			JLabel name = new JLabel(target.localParams.get(i).name);
		...
		JButton sabButton = new JButton("Submit");
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < textArray.size(); i++) {
					switch (textArray.get(i).type) {
						...
						case "integer":
							try {
								int test = Integer.parseInt(textArray.get(i).textField.getText());
						...
						case "float":
							try {
								float test = Float.parseFloat(textArray.get(i).textField.getText());
								target.localParams.get(i).value = "" + test;
							} catch (NumberFormatException ie) {
							    textArray.get(i).textField.setText("invalid format");
							}
